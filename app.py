...
# ---------- YOUR EMAIL CREDENTIALS (CHANGE THESE) ----------
SENDER_EMAIL = "your_email@gmail.com"      # chrissaahfallah21@gmail.com
SENDER_PASSWORD = "your_app_password"      # qvubuosqkphdmio(no spaces)
# ------------------------------------------------------------
...
# ---------- Robust price fetcher with fallback APIs ----------
...
<head>
    ...
    <style>
        body { font-family: Arial; max-width: 500px; margin: 50px auto; padding: 20px; background: #f4f4f9; }
        ...
        .alert-list { margin-top: 30px; border-top: 1px solid #ddd; padding-top: 20px; }
    ...
    <div class="box">
        ...
        <div class="alert-list">
            ...
            <div class="alert-item">
                ...
                <br><small>Email: {{ alert.email }} | Set: {{ alert.created_at }}</small>
                <form method="POST" action="/delete/{{ alert.id }}" style="display:inline;">
                    <button type="submit" style="background:#e74c3c; padding:4px 12px; width:auto; font-size:12px;">Delete</button>
                </form>
            </div>
            {% endfor %}
        </div>
...
def index():
    ...
    return render_template_string(HTML, message="", message_type="", alerts=alert_list)
...
def set_alert():
    ...
    conn = sqlite3.connect('alerts.db')
    c = conn.cursor()
    c.execute('INSERT INTO alerts (email, coin, target_price) VALUES (?, ?, ?)',
              (email, coin, target))
    alert_id = c.lastrowid
    conn.commit()
    conn.close()
    
    # 2. Check current price immediately
    current = get_current_price(coin)
    if current is None:
        message = "Could not fetch price. Alert saved but not checked."
        msg_type = "error"
    else:
        if current >= target:
            success = send_alert_email(email, coin, target, current)
            if success:
                conn = sqlite3.connect('alerts.db')
                c = conn.cursor()
                ...
                conn.commit()
                conn.close()
                message = f"✅ Triggered! {coin} is ${current:.2f}. Email sent to {email}."
                msg_type = "success"
            else:
                message = f"⚠️ Price hit, but email failed. Check your credentials."
                msg_type = "error"
        else:
            message = f"⏳ Waiting. {coin} is currently ${current:.2f}. Target is ${target}. We'll check hourly."
            msg_type = "info"
    
    # Fetch updated alerts for display
    conn = sqlite3.connect('alerts.db')
    c = conn.cursor()
    c.execute('SELECT id, email, coin, target_price, triggered, created_at FROM alerts ORDER BY created_at DESC')
    rows = c.fetchall()
    ...
    return render_template_string(HTML, message=message, message_type=msg_type, alerts=alert_list)
